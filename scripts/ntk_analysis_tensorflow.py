...
def find_first_batch_compatible_array(npz_dict: Dict[str, np.ndarray]) -> Tuple[str, np.ndarray]:
    """Pick the first array that can be interpreted as one batch or many batches."""
    for key, value in npz_dict.items():
        value = np.asarray(value)
        if value.ndim in {1, 2, 3}:
            return key, value
    raise ValueError("No 1D/2D/3D array found in the provided .npz file.")
...
def normalize_batch_array(arr: np.ndarray) -> np.ndarray:
    """Normalize one batch to shape [batch, features]."""
    arr = np.asarray(arr, dtype=np.float64)
    if arr.ndim == 1:
        arr = arr.reshape(-1, 1)
    return maybe_transpose_batch(arr)


def sanitize_label(label: str) -> str:
    """Turn free-form labels into filesystem-friendly names."""
    sanitized = re.sub(r"[^A-Za-z0-9._-]+", "_", label).strip("._")
    return sanitized or "batch"


def extract_batches_from_array(array: np.ndarray, base_label: str) -> Tuple[List[np.ndarray], List[str]]:
    """Extract one or many 2D batches from a 1D/2D/3D array."""
    array = np.asarray(array, dtype=np.float64)

    if array.ndim in {1, 2}:
        return [normalize_batch_array(array)], [base_label]

    if array.ndim == 3:
        batches = [normalize_batch_array(array[idx]) for idx in range(array.shape[0])]
        labels = [f"{base_label}_{idx:03d}" for idx in range(array.shape[0])]
        return batches, labels

    raise ValueError(
        f"Expected a 1D/2D/3D batch array, got shape {array.shape}."
    )
...
def load_input_batches(
    ...
    batch_size: int,
    input_dim: int,
    load_all_batches: bool,
) -> LoadedBatchCollection:
    """
    Load one batch or many batches.

    When `load_all_batches` is enabled, 3D arrays are treated as stacks of
    batches and NPZ archives are scanned for every compatible array.
    """
    if path is None:
        batch = make_synthetic_batch(batch_size=batch_size, input_dim=input_dim)
        return LoadedBatchCollection(
            batches=[batch],
            source="synthetic",
            description=(
    ...
    if suffix == ".npy":
        ...
        if load_all_batches:
            batches, labels = extract_batches_from_array(array, base_label=data_path.stem)
            return LoadedBatchCollection(
                batches=batches,
                source=str(data_path),
                description=f"Loaded {len(batches)} batch(es) from {data_path}",
                labels=labels,
            )
        if np.asarray(array).ndim == 3:
            warnings.warn(
                f"{data_path} contains a 3D array. Only the first batch is used because "
                "`--all-batches` was not specified.",
                stacklevel=2,
            )
            batch = np.asarray(array)[0]
        else:
            batch = array
    elif suffix == ".npz":
        archive = {k: np.asarray(v) for k, v in np.load(data_path, allow_pickle=True).items()}
        if load_all_batches:
            keys = [input_key] if input_key is not None else list(archive.keys())
            batches: List[np.ndarray] = []
            labels: List[str] = []
            skipped_keys: List[str] = []
            for key in keys:
                if key not in archive:
                    raise KeyError(f"`{key}` not found in NPZ archive {data_path}.")
                value = np.asarray(archive[key])
                if value.ndim not in {1, 2, 3}:
                    skipped_keys.append(key)
                    continue
                extracted_batches, extracted_labels = extract_batches_from_array(value, base_label=key)
                batches.extend(extracted_batches)
                labels.extend(extracted_labels)
            if not batches:
                raise ValueError(
                    f"No compatible 1D/2D/3D arrays found in {data_path}. "
                    f"Skipped keys: {skipped_keys}"
                )
            description = f"Loaded {len(batches)} batch(es) from NPZ archive {data_path}"
            if skipped_keys:
                description += f". Skipped non-batch keys: {skipped_keys}"
            return LoadedBatchCollection(
                batches=batches,
                source=str(data_path),
        ...
        if input_key is None:
            input_key, batch = find_first_batch_compatible_array(archive)
        else:
            batch = np.asarray(archive[input_key])
        if np.asarray(batch).ndim == 3:
            warnings.warn(
                f"Key `{input_key}` in {data_path} contains a 3D array. Only the first batch is used because "
                "`--all-batches` was not specified.",
                stacklevel=2,
            )
    ...
    batch = normalize_batch_array(batch)
    return LoadedBatchCollection(
        batches=[batch],
        source=str(data_path),
        description=f"Loaded real batch from {data_path}",
        labels=[sanitize_label(data_path.stem if input_key is None else input_key)],
    )


def load_input_batch(path: Optional[str], input_key: Optional[str], batch_size: int, input_dim: int) -> LoadedBatch:
    """Backward-compatible wrapper that returns only the first loaded batch."""
    collection = load_input_batches(
        path=path,
        input_key=input_key,
...
def compute_empirical_ntk(model: tf.keras.Model, batch: np.ndarray) -> Tuple[np.ndarray, np.ndarray, Dict[str, object]]:
    """
    ...
    layer_parameter_counts: List[int] = []
    layer_sample_energy: List[tf.Tensor] = []
    ...
    for layer in model.layers:
        ...
        layer_parameter_counts.append(
            int(np.sum([int(np.prod(variable.shape)) for variable in layer.trainable_variables]))
        )
        energy = tf.zeros((tf.shape(outputs)[0],), dtype=outputs.dtype)
        ...
        for variable in layer.trainable_variables:
            ...
            jac_flat = flatten_jacobian_block(jac)
            jacobian_blocks.append(jac_flat)
            energy += tf.reduce_sum(tf.square(jac_flat), axis=[1, 2])

        layer_sample_energy.append(tf.sqrt(energy))
    del tape
    ...
    layer_sensitivity = tf.stack(layer_sample_energy, axis=1) if layer_sample_energy else tf.zeros((tf.shape(outputs)[0], 0))
    return ntk.numpy(), full_jacobian.numpy(), {
        "layer_names": layer_names,
        "layer_parameter_counts": layer_parameter_counts,
        "sample_layer_sensitivity": layer_sensitivity.numpy(),
    }
...
def plot_model_block_sensitivity_heatmap(
    matrix: np.ndarray,
    layer_names: Sequence[str],
    path: Path,
    title: str = "Sensitivity heatmap over all trainable model blocks",
) -> None:
    """
    Plot a heatmap where rows are samples and columns are trainable blocks.

    Each entry stores the L2 norm of the gradient contribution of one trainable
    block for one sample, so the figure exposes how strongly different parts of
    the full model participate in the NTK construction.
    """
    fig_width = max(7.5, 1.25 * max(1, len(layer_names)))
    fig_height = max(5.0, 0.35 * max(1, matrix.shape[0]) + 2.0)
    ...
    if HAS_SEABORN:
        sns.heatmap(
            ...
            ax=ax,
            cmap="magma",
            xticklabels=list(layer_names),
            yticklabels=np.arange(matrix.shape[0]),
            cbar_kws={"label": r"$\|\partial f(x_i) / \partial \theta_\ell\|_2$"},
        )
    else:
        image = ax.imshow(matrix, cmap="magma", aspect="auto")
        cbar = fig.colorbar(image, ax=ax)
        cbar.set_label(r"$\|\partial f(x_i) / \partial \theta_\ell\|_2$")
        ax.set_xticks(np.arange(len(layer_names)))
        ax.set_xticklabels(list(layer_names), rotation=30, ha="right")
        ax.set_yticks(np.arange(matrix.shape[0]))
        ax.set_yticklabels(np.arange(matrix.shape[0]))

    ax.set_title(title)
    ax.set_xlabel("Trainable block of the model")
...
def plot_heatmap_overview(
    ...
    """Save a small-multiples overview for every batch NTK heatmap."""
    if not matrices:
        return
    ...
    for ax in axes.flat[num_matrices:]:
        ax.axis("off")
...
def validate_batch_shape(batch: np.ndarray, builder: str, input_dim: int) -> None:
    """Validate that a batch matches the expected input dimensionality."""
    expected_input_dim = 2 if builder == "recovered_pinn" else input_dim
    if batch.ndim != 2 or batch.shape[1] != expected_input_dim:
        raise ValueError(
            f"Input batch shape {batch.shape} is incompatible with builder `{builder}`. "
...
def analyze_single_batch(
    ...
    """Run the full NTK analysis pipeline for one batch and save all artifacts."""
    predictions = model(tf.convert_to_tensor(batch, dtype=dtype), training=False).numpy()
    ntk_matrix, full_jacobian, model_sensitivity_info = compute_empirical_ntk(model=model, batch=batch)
    ...
    plot_heatmap(
        ...
        title=f"Empirical NTK heatmap ({batch_label})",
        cmap="viridis",
    )
    ...
    plot_model_block_sensitivity_heatmap(
        model_sensitivity_info["sample_layer_sensitivity"],
        model_sensitivity_info["layer_names"],
        path=output_dir / "ntk_model_block_heatmap.png",
    )
    ...
    save_array(output_dir / "ntk_model_block_sensitivity.npy", model_sensitivity_info["sample_layer_sensitivity"])
    ...
    report = {
        ...
        "metrics": metrics,
        "model_sensitivity": {
            "layer_names": model_sensitivity_info["layer_names"],
            "layer_parameter_counts": model_sensitivity_info["layer_parameter_counts"],
            "matrix_shape": list(model_sensitivity_info["sample_layer_sensitivity"].shape),
        },
        "artifacts": {
            "heatmap": str(output_dir / "ntk_heatmap.png"),
            "eigenvalues": str(output_dir / "ntk_eigenvalues.png"),
            "rows_pca": str(output_dir / "ntk_rows_pca.png"),
            "clustered_heatmap": str(output_dir / "ntk_clustered_heatmap.png") if cluster_order is not None else None,
            "model_block_heatmap": str(output_dir / "ntk_model_block_heatmap.png"),
            "model_block_sensitivity": str(output_dir / "ntk_model_block_sensitivity.npy"),
            "ntk_matrix": str(output_dir / "ntk_matrix.npy"),
            "predictions": str(output_dir / "model_predictions.npy"),
            "jacobian": str(output_dir / "jacobian_tensor.npy"),
        },
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If set, analyze every batch that can be extracted from the input file. "
            "For 3D arrays the first axis is treated as the batch index; for NPZ archives "
            "all compatible 1D/2D/3D arrays are scanned."
        ),
    )
    parser.add_argument(
        "--input-dim",
...
def main() -> None:
    ...
    batch_collection = load_input_batches(
        path=args.input_batch,
        input_key=args.input_key,
        batch_size=args.batch_size,
        input_dim=args.input_dim,
        load_all_batches=args.all_batches,
    )

    batch_reports: List[Dict[str, object]] = []
    overview_matrices: List[np.ndarray] = []
    overview_labels: List[str] = []

    for batch_index, (batch, batch_label) in enumerate(zip(batch_collection.batches, batch_collection.labels)):
        batch = np.asarray(batch, dtype=np.float64)
        validate_batch_shape(batch=batch, builder=args.builder, input_dim=args.input_dim)

        if len(batch_collection.batches) == 1:
            batch_output_dir = output_dir
        else:
            batch_output_dir = ensure_dir(
                output_dir / "batches" / f"{batch_index:03d}_{sanitize_label(batch_label)}"
            )

        analysis_result = analyze_single_batch(
            model=model,
            batch=batch,
        ...
        batch_report = analysis_result["report"]
        batch_report["output_dir"] = str(batch_output_dir)
        batch_reports.append(batch_report)
        overview_matrices.append(analysis_result["ntk_matrix"])
        overview_labels.append(batch_label)
    ...
    report = {
        ...
        "batch_info": {
            "source": batch_collection.source,
            "description": batch_collection.description,
            "num_batches": len(batch_collection.batches),
            "labels": batch_collection.labels,
            "shapes": [report_item["batch_shape"] for report_item in batch_reports],
        },
    ...
    if len(batch_collection.batches) > 1:
        plot_heatmap_overview(
            overview_matrices,
            overview_labels,
            path=output_dir / "ntk_heatmaps_all_batches.png",
        )
        report["artifacts"] = {
            "heatmap_overview": str(output_dir / "ntk_heatmaps_all_batches.png"),
            "batch_root": str(output_dir / "batches"),
        }
    ...
    print(f"\nBatch source: {batch_collection.source}")
    print(batch_collection.description)
    print(f"Loaded batches: {len(batch_collection.batches)}")
    for batch_report in batch_reports:
        print(f"\n=== BATCH {batch_report['batch_label']} ===")
        print(f"Batch shape: {tuple(batch_report['batch_shape'])}")
        print_report(batch_report["metrics"])
    ...
    if len(batch_collection.batches) == 1:
        ...
        print(f"  - {output_dir / 'ntk_model_block_heatmap.png'}")
        if batch_reports[0]["cluster_order"] is not None:
            print(f"  - {output_dir / 'ntk_clustered_heatmap.png'}")
        print(f"  - {output_dir / 'ntk_report.json'}")
        print(f"  - {output_dir / 'ntk_matrix.npy'}")
    else:
        print("Main files:")
        print(f"  - {output_dir / 'ntk_heatmaps_all_batches.png'}")
        print(f"  - {output_dir / 'ntk_report.json'}")
        print(f"  - {output_dir / 'batches'}")
