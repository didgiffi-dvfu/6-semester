...
\textit{В традиционной записи первая фундаментальная форма имеет вид}
$$ds^2 = E\,du^2 + 2F\,du\,dv + G\,dv^2,$$
\textit{где $E = g_{11}$, $F = g_{12}$, $G = g_{22}$; это функции координат $u,v$, зависящие от выбранной параметризации поверхности.}
\end{proposition}
...
\subsection{Главные кривизны и средняя кривизна}

\begin{definition}[Главные кривизны]
Главными кривизнами поверхности $S$ в точке $p$ называются экстремальные (наименьшая и наибольшая) нормальные кривизны $k_1$ и $k_2$. Соответствующие направления называются главными направлениями.
\end{definition}

\begin{itemize}
    \item \textbf{Гауссова кривизна:} $k_g = k_1 \cdot k_2$.
    \item \textbf{Средняя кривизна:} $k_{\text{ср}} = \dfrac{k_1 + k_2}{2}$.
\end{itemize}

\begin{theorem}[О вычислении главных кривизн]
Пусть $S$ --- регулярная поверхность, $p \in S$, а $[I_p] = G$ и $[II_p] = B$ --- матрицы первой и второй фундаментальных форм соответственно:
\[
[I_p] = G = \begin{pmatrix} g_{11} & g_{12} \\ g_{21} & g_{22} \end{pmatrix}, \qquad
[II_p] = B = \begin{pmatrix} b_{11} & b_{12} \\ b_{21} & b_{22} \end{pmatrix}.
\]
Тогда:
\begin{enumerate}
    \item Главные кривизны $k_1, k_2$ являются корнями характеристического уравнения
    \[
    \det(B - \lambda G) = 0.
    \]
    \item Гауссова кривизна $k_g$ равна отношению детерминантов
    \[
...
\begin{example}[Поверхность вращения]
Рассмотрим поверхность вращения, заданную параметрически:
\[
r(u, v) = \big(x(u)\cos v,\ x(u)\sin v,\ z(u)\big).
\]
Тогда
\[
r_u = \big(x'(u)\cos v,\ x'(u)\sin v,\ z'(u)\big),
\qquad
r_v = \big(-x(u)\sin v,\ x(u)\cos v,\ 0\big).
\]
Векторное произведение касательных векторов:
\[
r_u \times r_v = \big(-x(u)z'(u)\cos v,\ -x(u)z'(u)\sin v,\ x(u)x'(u)\big),
\]
а его длина равна
\[
...
\[
m = \left(
\frac{-z'(u)\cos v}{\sqrt{(x')^2 + (z')^2}},
\frac{-z'(u)\sin v}{\sqrt{(x')^2 + (z')^2}},
\frac{x'(u)}{\sqrt{(x')^2 + (z')^2}}
\right).
\]
Коэффициенты первой фундаментальной формы:
\[
g_{11} = (x'(u))^2 + (z'(u))^2,\qquad g_{12} = 0,\qquad g_{22} = (x(u))^2.
\]
Вторые производные:
\[
r_{uu} = \big(x''(u)\cos v,\ x''(u)\sin v,\ z''(u)\big),
\]
\[
r_{uv} = \big(-x'(u)\sin v,\ x'(u)\cos v,\ 0\big),
\]
\[
r_{vv} = \big(-x(u)\cos v,\ -x(u)\sin v,\ 0\big).
\]
Коэффициенты второй фундаментальной формы:
\[
b_{11} = \frac{-x''(u)z'(u) + x'(u)z''(u)}{\sqrt{(x')^2 + (z')^2}},\qquad
b_{12} = 0,\qquad
b_{22} = \frac{x(u)z'(u)}{\sqrt{(x')^2 + (z')^2}}.
\]
...
\subsection{Геодезическая кривизна и геодезические линии}

\begin{definition}[Нормальная и геодезическая кривизна]
Пусть на поверхности $S$ проведена кривая $C$. Ее вектор кривизны $k\mathbf{n}$ можно разложить на составляющие по направлениям нормали $\mathbf{m}$ к поверхности и касательной к поверхности $\mathbf{u}$:
\[
k\mathbf{n} = \vec{MN} + \vec{MG}.
\]
Вектор $\vec{MN}$ называется \textit{вектором нормальной кривизны}, а вектор $\vec{MG}$ --- \textit{вектором геодезической кривизны}. Их длины называются соответственно нормальной и геодезической кривизной кривой $C$ на поверхности $S$.
\end{definition}

\begin{proposition}[Геометрический смысл]
Вектор нормальной кривизны совпадает с вектором кривизны нормального сечения с той же касательной. Вектор геодезической кривизны совпадает с вектором кривизны ортогональной проекции кривой на касательную плоскость к поверхности в данной точке.
\end{proposition}

\begin{proposition}[Вычисление геодезической кривизны]
Если $\mathbf{r}=\mathbf{r}(u^1,u^2)$, а текущие координаты на кривой считаются функциями длины дуги $s$, то
\[
\dot{\mathbf{r}} = \mathbf{r}_1 \dot{u}^1 + \mathbf{r}_2 \dot{u}^2,
\]
\[
\ddot{\mathbf{r}} = \Gamma_{ij}^1 \dot{u}^i \dot{u}^j \mathbf{r}_1 + \Gamma_{ij}^2 \dot{u}^i \dot{u}^j \mathbf{r}_2 + b_{ij} \dot{u}^i \dot{u}^j \mathbf{m} + \mathbf{r}_1 \ddot{u}^1 + \mathbf{r}_2 \ddot{u}^2.
\]
Отсюда
\[
\vec{MG} = \left( \ddot{u}^1 + \Gamma_{ij}^1 \dot{u}^i \dot{u}^j \right) \mathbf{r}_1 + \left( \ddot{u}^2 + \Gamma_{ij}^2 \dot{u}^i \dot{u}^j \right) \mathbf{r}_2,
\]
и
\[
k_g = \sqrt{g_{11}g_{22} - g_{12}^2} \, \left| \dot{u}^1 \left( \ddot{u}^2 + \Gamma_{ij}^2 \dot{u}^i \dot{u}^j \right) - \dot{u}^2 \left( \ddot{u}^1 + \Gamma_{ij}^1 \dot{u}^i \dot{u}^j \right) \right|.
\]
\end{proposition}

\begin{definition}[Геодезическая линия]
Геодезической линией на поверхности называется кривая, геодезическая кривизна которой в каждой точке равна нулю. Эквивалентно, для геодезической необходимо и достаточно, чтобы ее главная нормаль во всех точках совпадала с нормалью к поверхности, либо чтобы кривая была прямой.
\end{definition}

\begin{theorem}[Уравнения геодезических]
Пусть $u^1,u^2$ --- текущие координаты вдоль геодезической, рассматриваемые как функции длины дуги $s$. Тогда выполняются уравнения
\[
\frac{d^2u^1}{ds^2} = -\Gamma_{\alpha\beta}^1 \frac{du^\alpha}{ds}\frac{du^\beta}{ds}, \qquad
\frac{d^2u^2}{ds^2} = -\Gamma_{\alpha\beta}^2 \frac{du^\alpha}{ds}\frac{du^\beta}{ds}.
\]
Если геодезическая задается как $u^2=f(u^1)$, то
\[
f'' = -\Gamma_{11}^2 - (2\Gamma_{12}^2 - \Gamma_{11}^1) f' + (-\Gamma_{22}^2 + 2\Gamma_{12}^1) f'^2 + \Gamma_{22}^1 f'^3.
\]
Через каждую точку поверхности по каждому направлению проходит одна и только одна геодезическая.
\end{theorem}

\begin{proposition}[Геодезические на поверхности вращения]
Пусть $\rho$ --- расстояние от точки до оси вращения, а $\alpha$ --- угол между геодезической и параллелью поверхности вращения. Тогда вдоль каждой геодезической линии на поверхности вращения
\[
\rho \cos\alpha = \text{const}.
\]
В частности, при $\rho \cos\alpha = 0$ геодезическая представляет собой меридиан поверхности вращения.
\end{proposition}

\begin{theorem}[Кратчайшее свойство геодезических]
Отрезок геодезической $AB$ между точками $A$ и $B$ на ней дает кратчайшее расстояние между $A$ и $B$ по сравнению со всеми другими кривыми $AB$ на поверхности, соединяющими эти точки, если точка $B$ достаточно близка к $A$.
Если первая фундаментальная форма записана как
\[
ds^2 = E\,du^2 + 2F\,du\,dv + G\,dv^2,
\]
то длина кривой выражается интегралом
\[
\int\limits_A^B \sqrt{E\,du^2 + 2F\,du\,dv + G\,dv^2}.
\]
Экстремум этого функционала приводит к дифференциальному уравнению геодезической как к уравнению Эйлера--Лагранжа.
\end{theorem}
