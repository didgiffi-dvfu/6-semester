...
\begin{titlepage}
    ...
    \textbf{ИНДИВИДУАЛЬНОЕ ДОМАШНЕЕ ЗАДАНИЕ №16}\\[0.3cm]
...
Условие:
\begin{itemize}
    \item дано: выборка объёма \(n=100\) из непрерывной случайной величины \(X_{16}\);
...
\input{tables/task1_interval_table.tex}
...
\begin{itemize}
    ...
    \item требуется: найти оценку \(\alpha\) методом максимального правдоподобия и проверить её состоятельность и несмещённость.
\end{itemize}
...
\begin{lstlisting}
def gamma_mle_alpha(sample, lam):
    x_bar = float(np.mean(sample))
    return lam / x_bar

# Состоятельность:
# xbar -> E[X] = lam / alpha, hence alpha hat -> alpha.
# Несмещённость:
# E[alpha hat] = alpha * (n*lam)/(n*lam - 1), so the estimator is biased.
\end{lstlisting}
...
Условие:
\begin{itemize}
    \item дано: выборка объёма \(n=36\) из нормальной случайной величины;
...
\input{tables/task3_results_table.tex}
...
\begin{itemize}
    ...
    \item требуется: найти вероятность того, что относительная погрешность оценки дисперсии не превышает \(0.3\).
\end{itemize}
...
\input{tables/task4_results_table.tex}
...
Условие:
\begin{itemize}
    \item дано: частоты попаданий в интервалы и гипотетическая плотность;
    \item требуется: проверить гипотезу по критерию Пирсона при \(\alpha=0.01\) и указать достигнутый уровень значимости.
\end{itemize}
...
\input{tables/task5_results_table.tex}
...
\input{tables/task6_results_table.tex}
...
Условие:
\begin{itemize}
    \item дано: 40 пар наблюдений \((X,Y)\);
...
Проверка значимости коэффициентов выполняется по статистике Стьюдента.
Для каждого коэффициента используем
\[
...
\input{tables/task7_coefficients_table.tex}
\input{tables/task7_model_table.tex}
...
В работе были решены все пункты индивидуального задания 16:
построены интервальные ряды, ЭФР, доверительные интервалы, выполнены проверки гипотез и построены регрессионные модели.
Численные расчёты, таблицы и графики подготовлены в скрипте \texttt{idz\_code.py}.
